...
func Loader(name string, metadata *chart.Metadata) (loader.ChartLoader, error) {
	...
	if fi.IsDir() {
		return newDirLoader(name, metadata), nil
	}
...
func LoadFiles(files []*BufferedFile, metadata *chart.Metadata) (*chart.Chart, error) {
	...
	// metadata is not read from Chart.yaml
	if c.Metadata == nil {
		c.Metadata = metadata
	}

	// subcharts's metadata is read from Chart.yaml
	if c.Metadata == nil {
	...
	for n, files := range subcharts {
		...
		case filepath.Ext(n) == ".tgz":
			...
			// the dependencies chart must have chart.yaml
